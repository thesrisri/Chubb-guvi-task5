...
function paginationData(dataSet, page, rows) {
  var start = (page - 1) * rows;
  var end = start + rows;

  var slicedData = dataSet.slice(start, end);
...
function buttons(pages) {
  var pageValue = document.getElementById("pagination");
  pageValue.innerHTML =
    `<button value=${1} class="page btn btn-sm btn-info">First</button>` +
    `<button value=${
      state.page - 1
    } class="page btn btn-sm btn-info">Previous </button>`;

  for (var i = 1; i < pages + 1; i++) {
    pageValue.innerHTML += `<button value=${i} class="page btn btn-sm btn-info">${i}</button>`;
  }
...
function tableFunction() {
  ...
  var data = paginationData(state.dataSet, state.page, state.rows);
